function downloadShibe() {
    ...
    //Objeto que representa la petición (request)
    const charge = document.getElementById("imagen");
    charge.style = "display:block;";
    xhttp.onreadystatechange = function() {
        //Evento cuando el estado haya cambiado (cuando esté listo) Se ejecutará cuando esté lista, cuando reciba la respuesta
        if (this.readyState == 4 && this.status == 200) {
            //Todas las respuestas “200 y algo” serán respuestas satisfactorias
            charge.style = "display:none;";
            const dogeResponse = JSON.parse(this.responseText);
            //No usar funciones flechas cuando usamos this.  usar json punto stringify cuando tratemos con objetos 
